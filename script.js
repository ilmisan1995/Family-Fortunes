...
let database = [
    {
        q: ["APA YANG BIASA ADA DI DOMPET?", ""],
        a: [
            { text: "UANG", points: 45 },
            { text: "KTP", points: 25 },
            { text: "FOTO", points: 15 },
            { text: "STNK", points: 10 },
            { text: "KARTU ATM", points: 5 }
        ]
    }
];
...
function renderGrid(answers) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        // Answer Zone
        if (i < answers.length) {
            grid.innerHTML += `<div class="answer-container" id="ans-${i}" data-text="${answers[i].text}" data-pts="${answers[i].points}">
                <span>${i+1}. ___________________</span><span>--</span></div>`;
        } else {
    ...
    // Baris 10: Total
    grid.innerHTML += `<div class="empty"></div><div class="answer-container revealed" style="border:none; border-top:2px double var(--led-yellow); margin-top:10px;">
        <span>TOTAL SKOR (x${multiplier})</span><span id="grand-total">000</span></div><div class="empty"></div>`;
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (key === 'M') backToMenu();
    if (key === 'N') { if(currentRoundIndex < database.length-1) {currentRoundIndex++; initRound();}};
    if (key === 'P') { if(currentRoundIndex > 0) {currentRoundIndex--; initRound();}};
    if (key === '9') updateTeamScore(1);
    if (key === '0') updateTeamScore(2);
...
function toggleEditMode() {
    ...
    if(p.style.display === 'flex') {
        ...
        container.innerHTML = '';
        for(let i=1; i<=8; i++) {
            container.innerHTML += `<div style="display:flex; gap:10px; margin-bottom:5px;">
            <input type="text" id="ed-t-${i}" placeholder="Jawaban ${i}" maxlength="35">
            <input type="number" id="ed-p-${i}" placeholder="Skor" style="width:60px"></div>`;
        }
    }
}
